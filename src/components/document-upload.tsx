...
export function DocumentUpload({
		...
		(e: React.DragEvent) => {
			...
			if (
				...
				(allowedTypes.includes(file.type) ||
					/\.(pdf|txt|mp3|wav|m4a|aac|ogg|mp4|mov)$/i.test(file.name))
			) {
				// 파일 크기 검증 (10MB = 10 * 1024 * 1024 bytes)
				const maxSize = 10 * 1024 * 1024;
				if (totalUploadedSize + file.size > maxSize) {
	...
	const removeFile = () => {
		if (uploadedFile) {
			setTotalUploadedSize((prev) => prev - uploadedFile.size);
		}
						...
						>
							<div className="text-center">
								...
								<p className="text-muted-foreground mb-6">
									PDF, TXT, MP3, WAV, MP4 등 오디오/영상 파일도 지원합니다
								</p>
